...
// import { BlogComment, BlogReply } from "@/app/interfaces/interfaces";
...
interface BlogsCommentSectionProps {
  slug: string;
  blogId: string;
}

const BlogsCommentSection = ({ slug, blogId }: BlogsCommentSectionProps) => {
  ...
  const [blogLikes, setBlogLikes] = useState<mongoose.Types.ObjectId[]>([]);
  const [blogLikesCount, setBlogLikesCount] = useState(0);
  const [showLikesModal, setShowLikesModal] = useState(false);
  const [likeUsers, setLikeUsers] = useState<any[]>([]);
  const [isLoadingUsers, setIsLoadingUsers] = useState(false);

  // Fetch comments and blog likes for the blog
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (blogResponse.data.data) {
          ...
          setBlogLikes(blogResponse.data.data.likes || []);
          setBlogLikesCount((blogResponse.data.data.likes || []).length);
        }
      } catch (err) {
  ...
  //     setLoading(true);
  //     try {
  //       const response = await axios.post(`/api/blogs/${slug}/comments`, {
  ...
  //       const newCommentData = response.data.comment as BlogComment;
  ...
  //         })
  //       );
  //       await axios.post(`/api/blogs/${slug}/comments/${commentId}/likes/`, {
  //         parentId: slug,
  //         parentType: "blog",
  //       });
  //     } catch (err) {
  ...
  //         `/api/blogs/${slug}/comments/${commentId}/replies`,
  //         {
  //           text: replyText.trim(),
  //         }
  //       );

  //       // Cast the reply to BlogReply type and ensure it matches the structure of BlogComment
  //       const newReply = response.data.reply as BlogReply;

  //       // Enhance the reply with user data to match BlogComment structure
  //       const enhancedReply = {
  //         ...newReply,
  //         userId:
  ...
  //       );
  //       await axios.post(
  //         `/api/blogs/${slug}/comments/${commentId}/replies/${replyId}/likes`,
  //         {
  //           parentId: slug,
  //           parentType: "blog",
  //         }
  ...
  const fetchLikeUsers = async () => {
    ...
    try {
      const response = await axios.get(`/api/blogs/${slug}/likes/users`);
      setLikeUsers(response.data.users || []);
    } catch (err) {
      console.error("Error fetching like users:", err);
      setError("Failed to load users who liked this blog");
    } finally {
      setIsLoadingUsers(false);
    }
  };

  // Toggle like on the blog
  const handleLikeBlog = async () => {
    if (!session?.user) {
      setError("You must be logged in to like blogs");
      return;
    }
    ...
    try {
      ...
      const userId = session.user.id;
      if (userId) {
        const alreadyLiked = blogLikes.some((id) => id.toString() === userId);

        if (alreadyLiked) {
          setBlogLikes(blogLikes.filter((id) => id.toString() !== userId));
          setBlogLikesCount((prev) => prev - 1);
        } else {
          setBlogLikes([...blogLikes, new mongoose.Types.ObjectId(userId)]);
          setBlogLikesCount((prev) => prev + 1);
        }
      }
      await axios.post(`/api/blogs/${slug}/likes/`, {
        parentId: blogId,
        parentType: "blog",
      });
    } catch (err) {
  ...
  //     try {
  //       await axios.delete(`/api/blogs/${slug}/comments/${commentId}/delete`);
  ...
  //     try {
  //       await axios.delete(
  //         `/api/blogs/${slug}/comments/${commentId}/replies/${replyId}/delete`
  //       );
      ...
      <div className="flex md:hidden justify-end w-full  items-center gap-2">
        <button
          ...
          className={`flex items-center gap-1 px-3 py-2 rounded-full transition-all duration-300 ${
            session?.user &&
            session.user.id &&
            blogLikes.some((id) => id.toString() === session.user.id)
        ...
        {blogLikesCount === 0 && (
          <p className="hidden sm:block text-lovely">
            Be the first one to love this blog.
          </p>
        )}
        {blogLikesCount > 0 && (
          <button
          ...
          >
            {blogLikesCount}{" "}
            {blogLikesCount === 1
              ? "person love this blog"
              : "people love this blog"}
          </button>
        )}
      </div>
...
export default BlogsCommentSection;
