...
const CartItemSmall = ({item,wishListBool}:{item:CartItem,wishListBool:boolean}) => {
  ...
  const { updateQuantity, addItem } = useCart();
    ...
    if (opr === '-') {
      if (quantity > 1) {
        ...
        if (wishListBool === true) {
          ...
          updateQuantity(productId, updatedQuantity, item.variant, item.attributes);
        }
      } else if (quantity === 1) {
        // Remove item when quantity reaches 0
        if (wishListBool === true) {
          const updatedWishList = wishList.filter((wishItem) => 
            !(wishItem.productId === productId && 
              wishItem.attributes.name === item.attributes.name &&
              wishItem.variant.name === item.variant.name)
          );
  ...
  const transferItemToCart = () => {
    ...
    <div className='text-creamey w-full bg-everGreen rounded-2xl px-2 py-2'>
        ...
        <div className="flex w-full h-full  py-2  gap-4">
          <div  className='cursor-pointer h-full flex justify-start items-start'><span 
          // onClick={
          //   ()=>deleteItem(item.productId,item.size,item.color)
          //   } 
            className='text-[12px] text-creamey'>x</span></div>
     ...
     height={80}
        ...
        <div className='flex w-full flex-col  justify-between '>
         <div>
         <h3 className={`${thirdFont.className} font-semibold text-lg w-full `}>{item.productName}</h3>
          </div> 
          ...
          <div className="mt-0.5 flex gap-3 justify-between   font-semibold text-xs ">
            ...
            className='flex gap-4 items-center justify-between'>
              ...
              <span className='px-3 py-2 bg-creamey rounded-lg text-everGreen cursor-pointer p-3 text-sm' onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
      ...
      <span onClick={(e) => {
        ...
        e.stopPropagation();
        handleQuantity('+', item.productId);
      }} className='text-sm px-3 py-2 bg-creamey rounded-lg text-everGreen cursor-pointer'> +</span>
            </div>
