...
export async function POST(req: Request) {
  try {
    ...
    if (
      ...
      cartTotal < discount.conditions.minimumOrderAmount
    ) {
      return NextResponse.json(
        {
    ...
    // Check usage limit
    if (
      discount.conditions?.usageLimit &&
      discount.usageCount >= discount.conditions.usageLimit
    ) {
      return NextResponse.json(
    ...
    return NextResponse.json({
      ...
      discount: {
        ...
        description:
          discount.calculationType === "FREE_SHIPPING"
            ? "Free Shipping"
            : undefined,
      },
      cartTotal,
